-- 프렙(샤우팅) 수강신청 접수 시간창 **개시 시각 07:00 → 10:00** (마감은 19:00 그대로).
--
-- 왜: 운영 요청으로 신청 불가 시간대를 "19:00 ~ 익일 07:00"에서 **"19:00 ~ 익일 10:00"**으로 넓힌다.
--     정책 근거(심야·이른 아침 접수 = 그 시각의 입금 안내 SMS + 응대 불가 시간의 입금 대조)는 20260826215339 그대로다.
--
-- ⚠️ 20260827210254는 이미 원격에 적용됐으므로 **수정하지 않고 새 마이그레이션으로 덮어쓴다**
...
begin
  ...
  -- 접수 시간창 — KST 10:00~19:00. `not between 10 and 18`은 18:59:59까지 통과하고 19:00:00부터 막는다
  -- (앱 헬퍼의 반개구간 `m >= 600 and m < 1140`과 같은 경계).
  if extract(hour from (now() at time zone 'Asia/Seoul')) not between 10 and 18 then
    return 'closed';
  end if;
