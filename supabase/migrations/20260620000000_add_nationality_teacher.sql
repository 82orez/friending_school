-- 강사 국적(nationality) 필드 추가. 신청폼·강사 프로필 공통, 저장 값은 국가명 문자열.
alter table public.teacher_applications add column if not exists nationality text;
alter table public.profiles add column if not exists nationality text;

-- 승인 RPC 재선언 — profiles 채움 블록에 nationality coalesce 추가(experience 등과 동일 패턴).
