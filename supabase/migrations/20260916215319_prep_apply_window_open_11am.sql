-- 프렙(샤우팅) 수강신청 접수 시간창 **개시 시각 10:00 → 11:00** (마감은 19:00 그대로).
--
-- 왜: 운영 요청으로 접수 가능 시간을 **"매일 오전 11시~오후 7시"**로 좁힌다
--     (신청 불가 시간대는 "19:00 ~ 익일 10:00" → **"19:00 ~ 익일 11:00"**).
--     정책 근거(심야·이른 아침 접수 = 그 시각의 입금 안내 SMS + 응대 불가 시간의 입금 대조)는 20260826215339 그대로다.
--
-- ⚠️ 20260906211254는 이미 원격에 적용됐으므로 **수정하지 않고 새 마이그레이션으로 덮어쓴다**
...
begin
  ...
  -- 접수 시간창 — KST 11:00~19:00. `not between 11 and 18`은 18:59:59까지 통과하고 19:00:00부터 막는다
  -- (앱 헬퍼의 반개구간 `m >= 600 and m < 1140`과 같은 경계).
  if extract(hour from (now() at time zone 'Asia/Seoul')) not between 11 and 18 then
    return 'closed';
  end if;
