...
// 진행 가이드 — 강좌 상세 모달의 「이용안내」 탭 문구. 강좌마다 다르지 않은 공통 안내라
// 강좌 행이 아니라 상수로 둔다(프렌더가 쓰는 강좌 소개와 역할이 다르다).
export const PREP_GUIDE = [
  "정해진 시간에 접속해서 큰 소리로 따라 읽어요.",
  "발음보다 리듬과 강세에 집중해요.",
  "매일 반복하면 입이 트여요.",
] as const;

// ── 수강신청 접수 시간창 ────────────────────────────────────────────────
// KST 11:00~19:00에만 신청을 받는다(19:00부터 익일 11:00까지 마감).
// 신청은 곧바로 신청자에게 입금 안내 SMS, 관리자 메일, 프렌더 SMS를 띄우므로
// 심야·이른 아침 접수는 그 시각의 문자 발송 + 응대 불가 시간대의 입금 대조를 뜻한다.
...
//    개시 시각 이력: 08:00(20260826220248) → 07:00(20260827210254) → 10:00(20260906211254) → **11:00(20260916215319)**.
export const PREP_APPLY_OPEN_MIN = 11 * 60;
export const PREP_APPLY_CLOSE_MIN = 19 * 60;

// 배너 안내와 서버 액션 에러 문구가 어긋나지 않도록 문장도 여기 한 곳에 둔다.
export const PREP_APPLY_WINDOW_LABEL = "오전 11시~오후 7시";
export const PREP_APPLY_CLOSED_MSG = `수강신청은 매일 ${PREP_APPLY_WINDOW_LABEL}에만 가능합니다.`;

// ── 입금 기한 ───────────────────────────────────────────────────────────
// 신청 당일 KST 21:00. 접수 시간창이 11:00~19:00이라 기한은 항상 신청 시점보다 뒤다(경계 예외 없음).
