// 프렙 새벽 일러스트 — 새벽 하늘 + 떠오르는 해 + Zoom 타일 + 6:00 시계. `/prep` 소개 페이지 히어로 전용.
//
...
//
// ⚠️ 한때 홈(/) 샤우팅 배너용 `variant="banner"`(납작한 viewBox·타일/시계 생략)가 있었는데,
//    그 배너가 사진 히어로(hero-shouting.jpg)로 바뀌면서 죽은 분기가 돼 걷어냈다.
export default function PrepHeroArt({ className }: { className?: string }) {
  ...
  return (
    <svg viewBox={`0 0 1200 ${height}`} preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false" className={className}>
      ...
      <rect width="1200" height={height} fill="url(#prep-sky)" />
      ...
      {/* 떠오르는 해 */}
      <circle cx="980" cy={sunY} r="190" fill="url(#prep-glow)" />
      <circle cx="980" cy={sunY} r="86" fill="url(#prep-sun)" />
