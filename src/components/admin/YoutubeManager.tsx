...
export default function YoutubeManager({ videos }: { videos: AdminVideo[] }) {
  ...
  return (
    <div>
      ...
      {error && <p role="alert" className="border-brand/30 bg-brand/5 text-brand mt-4 rounded-md border px-3.5 py-2.5 text-sm font-medium">{error}</p>}
      ...
      <div className="border-rule mt-5 rounded-xl border bg-white p-5">
        ...
        <Field className="mt-3" label="내용 설명" value={form.description} onChange={(v) => setForm({ ...form, description: v })} placeholder="영상 설명" textarea />
        <button
          type="button"
          disabled={pending}
          onClick={() => run(() => addYoutubeVideo(form), () => setForm(EMPTY))}
              ...
              <li key={v.id} className="border-rule border-b p-4 last:border-b-0 md:px-6">
                {editingId === v.id ? (
                  <div>
                    ...
                    <Field className="mt-3" label="영상 제목" value={editFields.title} onChange={(val) => setEditFields({ ...editFields, title: val })} />
                    <Field className="mt-3" label="내용 설명" value={editFields.description} onChange={(val) => setEditFields({ ...editFields, description: val })} textarea />
                    <div className="mt-3 flex gap-2">
                      <button
                        type="button"
                        disabled={pending}
                        onClick={() => run(() => updateYoutubeVideo(v.id, editFields), () => setEditingId(null))}
