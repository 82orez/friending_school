...
/* ===== 프렙 오늘 수업 안내 (docs/prep.md) ===== */

export type PrepReminderEmailData = {
  courseTitle: string;
  sessionNo: number;
  total: number;
  when: string; // "8월 26일 06:00~06:40"
  topic: string | null;
  enterUrl: string; // 마이페이지(입장 버튼이 있는 곳) — 메일에 zoom URL을 직접 싣지 않는다
};

/**
 * 수강생에게 '오늘 수업' 안내. 크론(/api/cron/prep-daily)이 하루 1회 호출한다.
 * best-effort — 실패해도 크론은 계속 진행한다(다른 수강생 발송을 막지 않는다).
 *
 * ⚠️ Zoom URL을 메일에 넣지 않는다: 전달·전달받은 메일로 미수강자가 입장할 수 있고,
 *    프렌더가 링크를 바꾸면 메일이 낡는다. 입장은 항상 서버가 자격·시간을 확인하는 화면을 거친다
 *    (enterPrepSession과 같은 원칙 — friender_rooms가 zoom_url을 저장하지 않는 이유와 동일).
 */
export async function sendPrepSessionReminder(to: string, data: PrepReminderEmailData): Promise<void> {
  const topicLine = data.topic?.trim() ? `<p style="font-size:14px;color:#1a1a1a;margin:0 0 4px">오늘 주제 · ${escapeHtml(data.topic)}</p>` : "";
  const html = `<div style="font-family:'Apple SD Gothic Neo',Arial,sans-serif;max-width:560px;margin:0 auto">
    <h2 style="font-size:18px;color:#1a1a1a;margin:0 0 4px">오늘 프렙 수업이 있어요</h2>
    <p style="font-size:14px;color:#666;margin:0 0 16px">${escapeHtml(data.courseTitle)} · ${data.sessionNo}/${data.total}강</p>
    <p style="font-size:15px;color:#1a1a1a;margin:0 0 4px"><strong>${escapeHtml(data.when)}</strong></p>
    ${topicLine}
    <p style="font-size:13px;color:#666;margin:16px 0 8px">시작 15분 전부터 마이페이지에서 입장할 수 있습니다.</p>
    <p style="margin:0 0 16px"><a href="${escapeHtml(data.enterUrl)}" style="display:inline-block;background:#1f3a93;color:#fff;font-size:14px;font-weight:bold;text-decoration:none;padding:10px 18px;border-radius:8px">마이페이지에서 입장하기</a></p>
    <p style="font-size:12px;color:#999;margin:0">프렌딩 스쿨</p>
  </div>`;
  const text = [
    "오늘 프렙 수업이 있어요.",
    "",
    `${data.courseTitle} · ${data.sessionNo}/${data.total}강`,
    data.when,
    data.topic?.trim() ? `오늘 주제: ${data.topic}` : "",
    "",
    "시작 15분 전부터 마이페이지에서 입장할 수 있습니다.",
    data.enterUrl,
  ]
    .filter(Boolean)
    .join("\n");
  await sendResultEmail([to], `[프렌딩 스쿨] 오늘 ${data.when} · ${data.courseTitle}`, html, text);
}
