...
export type TeacherApplyState = { error?: string };
...
export async function submitTeacherApplication(_prev: TeacherApplyState, formData: FormData): Promise<TeacherApplyState> {
  ...
  if (isFrienderRole(role)) return { error: "This account is registered as a friender." };
  ...
  // 제출 성공은 리다이렉트로 알린다 — 재검증되면 페이지가 'Under review' 분기로 바뀌며 폼이 언마운트돼
  ...
  redirect("/teacher/apply?submitted=1");
}
