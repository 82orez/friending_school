...
export type PrepCourseInput = {
  ...
  startMin: number;
  durationMin: number;
  sessionDates: string[]; // KST YYYY-MM-DD, 정확히 PREP_SESSION_COUNT개
};
...
function validatePrepInput(input: PrepCourseInput): {
  error?: string;
  values?: { title: string; description: string | null; level: string; capacity: number; startMin: number; durationMin: number; dates: string[] };
} {
  ...
  const raw = Array.isArray(input?.sessionDates) ? input.sessionDates : [];
  // Array.from — tsconfig target이 낮아 Set 스프레드는 빌드가 막힌다.
  const dates = Array.from(new Set(raw.filter((d) => typeof d === "string" && /^\d{4}-\d{2}-\d{2}$/.test(d)))).sort();
  if (dates.length !== raw.length) return { error: "수업 일자에 중복되거나 잘못된 날짜가 있습니다." };
  if (dates.length !== PREP_SESSION_COUNT) return { error: `수업 일자는 정확히 ${PREP_SESSION_COUNT}회여야 합니다.` };

  const today = todayKst();
  if (dates[0] <= today) return { error: "첫 수업은 내일 이후로 잡아 주세요." };
  if (dates[dates.length - 1] > addDays(today, PREP_MAX_AHEAD_DAYS)) {
    return { error: `수업 일자는 ${PREP_MAX_AHEAD_DAYS}일 이내로 선택해 주세요.` };
  }

  return { values: { title, description, level, capacity, startMin, durationMin, dates } };
}
...
export async function createPrepCourse(input: PrepCourseInput): Promise<PrepActionResult> {
  ...
  const { error: sessionError } = await admin
    .from("prep_sessions")
    .insert(v.values.dates.map((session_date, i) => ({ course_id: courseId, session_no: i + 1, session_date })));
