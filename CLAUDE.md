...
**"청년을 세계로"** 워홀·해외진출 영어 통합 플랫폼(브랜드 "프렌딩 스쿨"). 구성: **프렌딩 홈 `/`**(샤우팅 수강신청 배너 + 무료 연습방 목록 — 사이트 첫 화면) + **스쿨 소개 랜딩 `/school`**(5섹션, **admin 전용** — 라우트 가드 + 네비 탭 `isAdmin`) + **필리핀 화상영어 `/philippines-english`**·**액티비티 `/activities`**(각각 히어로 + `/school` 4·5번 섹션 공유 컴포넌트 `SpeakingDevelopSection`/`ActivitySection`) + **과정 상세페이지 5종**(`/courses/[slug]`: workhol·kitchen·grammar1·grammar2·cosmetic — 회화 기초문법 1/2는 각각 독립 과정·독립 수강신청, 교재 키 basic1/basic2) + **전자책 뷰어 `/textbook/[course]`**(레지스트리 기반 교재 5종, 무료 미리보기 외 로그인) + **수강신청→마이페이지**(`/courses/[slug]/enroll` 강사 매칭형 수강신청 → `/mypage`) + **강사 지원**(`/teacher/apply`) + **강사 페이지**(`/teacher`: 프로필·사진·zoom·주간 가능시간·수강신청 승인/거절) + **프렌더**(`/friender`: 연습방(**주간 스케줄 시리즈** — 시작일+요일+기간 1~8주로 회차 자동 생성, 예약·입장은 회차별)·받은 후기, Plus는 **샤우팅 유료 강좌** 개설 — **관리자 승인 후 개설 완료**, **수강신청은 홈 `/` 상단 배너**(무통장 — 별도 공개 소개 페이지 없음), **회차 Zoom 입장·출결은 「내 강의실」**) + **admin 대시보드**(`/admin`: 수강신청·화상수업·회원·강사 관리·**연습방 관리**·**샤우팅 수강신청**·센터·매출/매출이익/강사 정산·유튜브). Next.js 16(App Router, Turbopack) + React 19.2 + Tailwind v4. shadcn/ui(`base-nova`/`neutral`), Supabase SSR 인증(이메일 + 카카오 OAuth), 알림 메일(Resend) + SMS(Solapi).
...
- `docs/friender.md` — 프렌더 지원/승인·등급(friender/friender_plus)·**연습방 시리즈**(`/friender/rooms`)·프렌딩 홈(`/`)
- `docs/prep.md` — **샤우팅**(구 가칭 「프렙」 — 라우트·DB·코드 식별자는 `prep` 유지): 프렌더 Plus 유료 강좌(월 20회·평일 기본·캘린더 일자 조정) + **개설 심사**(작성중→신청→승인/거절, admin 탭 `/admin/prep`) + **수강신청**(입금대기→수강확정, 무통장). 카드결제·환불 미구현
- `docs/admin.md` — admin 대시보드 15탭(수강신청·화상수업·회원·강사/프렌더 승인·**연습방 모니터링/강제 삭제**·**샤우팅 강좌 심사**·**샤우팅 수강신청 관리**·센터·매출·매출이익·시뮬레이션·정산·유튜브·공지)
...
- **CTA/anchor**: 과정 목록은 `/philippines-english`(스쿨 소개는 admin 전용 탭), 과정 상세 `#apply-form` 등 anchor + `scroll-behavior:smooth`. `<a>` 버튼화는 `buttonVariants()`+`cn()`.
