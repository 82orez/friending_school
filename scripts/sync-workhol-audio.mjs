/**
 * One-time sync: download the workhol per-sentence audio clips from the live
 * reference site and upload them to Supabase Storage (public bucket).
 *
 * Source : https://survival-english-au.netlify.app/audio/<data-audio>
 * Target : bucket "textbook-audio", key "workhol/<data-audio>"
 *          → served at <SUPABASE_URL>/storage/v1/object/public/textbook-audio/workhol/<data-audio>
 *
 * The list of clips is derived from the `data-audio="..."` attributes in the
 * workhol unit HTML (content/textbook/workhol/unit-NN.html), so it always
 ...
 *
 * Run: node scripts/sync-workhol-audio.mjs
 */
...
const WORKHOL_DIR = path.join(ROOT, "content", "textbook", "workhol");
const LIVE_BASE = "https://survival-english-au.netlify.app/audio/";
const BUCKET = "textbook-audio";
const KEY_PREFIX = "workhol/";
const CONCURRENCY = 8;
...
function collectClips() {
  const set = new Set();
  for (const file of fs.readdirSync(WORKHOL_DIR).filter((f) => /^unit-\d+\.html$/.test(f))) {
    const html = fs.readFileSync(path.join(WORKHOL_DIR, file), "utf8");
...
async function main() {
  ...
  console.log(`Found ${clips.length} clips across workhol units. Ensuring bucket "${BUCKET}"...`);
  await ensureBucket();
