...
- **히어로 그림은 인라인 SVG `src/components/prep/PrepHeroArt.tsx`**(새벽 하늘·해·Zoom 타일·6:00 시계). ⚠️ 이제 **소비처가 이 페이지 하나**다 — 홈 배너용 `variant="banner"`(납작한 viewBox·타일/시계 생략)는 그 배너가 사진 히어로로 바뀌며 죽은 분기가 돼 제거했다. ⚠️ 파일 `.svg`를 `next/image`로 쓰려면 `images.dangerouslyAllowSVG`를 켜야 해서 장식 하나 때문에 그 스위치를 켜지 않았다(홈 `/` 히어로의 인라인 장식 SVG와 같은 선택). 색은 토큰과 같은 hex 리터럴(SVG gradient에 CSS 변수는 불안정).
